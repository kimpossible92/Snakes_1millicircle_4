...
namespace Photon.Pun.UtilityScripts
{
    ...
    /// <summary>
    /// Implements teams in a room/game with help of player properties. Access them by Player2.GetTeam extension.
    /// </summary>
    /// <remarks>
    ...
    public class PhotonTeamsManager : MonoBehaviour, IMatchmakingCallbacks, IInRoomCallbacks
    {
        ...
        private Dictionary<byte, HashSet<Player2>> playersPerTeam;
        ...
        public static event Action<Player2, PhotonTeam> PlayerJoinedTeam;
        public static event Action<Player2, PhotonTeam> PlayerLeftTeam;
        ...
        private void Init()
        {
            ...
            playersPerTeam = new Dictionary<byte, HashSet<Player2>>(teamsList.Count);
            for (int i = 0; i < teamsList.Count; i++)
            {
                teamsByCode[teamsList[i].Code] = teamsList[i];
                teamsByName[teamsList[i].Name] = teamsList[i];
                playersPerTeam[teamsList[i].Code] = new HashSet<Player2>();
            }
        }
        ...
        void IInRoomCallbacks.OnPlayerPropertiesUpdate(Player2 targetPlayer, Hashtable changedProps)
        {
            object temp;
            if (changedProps.TryGetValue(TeamPlayerProp, out temp))
            {
                ...
                {
                    Debug.LogErrorFormat("Unexpected: custom property key {0} should have of type byte, instead we got {1} of type {2}. Player2: {3}", 
                        TeamPlayerProp, temp, temp.GetType(), targetPlayer);
                }
            }
        }

        void IInRoomCallbacks.OnPlayerLeftRoom(Player2 otherPlayer)
        {
        ...
        void IInRoomCallbacks.OnPlayerEnteredRoom(Player2 newPlayer)
        {
        ...
        private void UpdateTeams()
        {
            ...
            {
                Player2 player = PhotonNetwork.PlayerList[i];
        ...
        /// <returns>If successful or not.</returns>
        public bool TryGetTeamMembers(byte code, out Player2[] members)
        {
            members = null;
            HashSet<Player2> players;
            if (this.playersPerTeam.TryGetValue(code, out players))
            {
                members = new Player2[players.Count];
                int i = 0;
        ...
        /// <param name="members">The array of players to be filled.</param>
        /// <returns>If successful or not.</returns>
        public bool TryGetTeamMembers(string teamName, out Player2[] members)
        {
            members = null;
            PhotonTeam team;
        ...
        /// <param name="members">The array of players to be filled.</param>
        /// <returns>If successful or not.</returns>
        public bool TryGetTeamMembers(PhotonTeam team, out Player2[] members)
        {
            members = null;
            if (team != null)
            {
        ...
        /// <returns>If successful or not.</returns>
        public bool TryGetTeamMatesOfPlayer(Player2 player, out Player2[] teamMates)
        {
            teamMates = null;
            if (player == null)
            {
            ...
            if (this.playersPerTeam.TryGetValue(team.Code, out players))
            {
                ...
                teamMates = new Player2[players.Count - 1];
                int i = 0;
        ...
        void IInRoomCallbacks.OnMasterClientSwitched(Player2 newMasterClient)
        {
        }
    ...
    /// <summary>Extension methods for the Player2 class that make use of PhotonTeamsManager.</summary>
    public static class PhotonTeamExtensions
    {
        ...
        public static PhotonTeam GetPhotonTeam(this Player2 player)
        {
            object teamId;
            PhotonTeam team;
        ...
        /// <returns></returns>
        public static bool JoinTeam(this Player2 player, PhotonTeam team)
        {
            if (team == null)
            {
        ...
        /// <returns></returns>
        public static bool JoinTeam(this Player2 player, byte teamCode)
        {
            PhotonTeam team;
        ...
        /// <returns></returns>
        public static bool JoinTeam(this Player2 player, string teamName)
        {
            PhotonTeam team;
        ...
        public static bool SwitchTeam(this Player2 player, PhotonTeam team)
        {
            if (team == null)
            {
        ...
        public static bool SwitchTeam(this Player2 player, byte teamCode)
        {
            PhotonTeam team;
        ...
        /// <returns>If the team switch request is queued to be sent to the server or done in case offline or not joined to a room yet.</returns>
        public static bool SwitchTeam(this Player2 player, string teamName)
        {
            PhotonTeam team;
        ...
        public static bool LeaveCurrentTeam(this Player2 player)
        {
            PhotonTeam currentTeam = player.GetPhotonTeam();
            if (currentTeam == null)
            {
        ...
        /// <returns>If successful or not.</returns>
        public static bool TryGetTeamMates(this Player2 player, out Player2[] teamMates)
        {
