...
namespace PathologicalGames
{
    ...
    /// </description>
    [AddComponentMenu("Path-o-logical/PoolManager2/Pre-Runtime Pool Item")]
    public class PreRuntimePoolItem : MonoBehaviour
    {
        ...
        {
            SpawnPool pool;
            if (!PoolManager2.Pools.TryGetValue(this.poolName, out pool))
            {

                string msg = "PreRuntimePoolItem Error ('{0}'): " +
                        "No pool with the name '{1}' exists! Create one using the " +
                        "PoolManager2 Inspector interface or PoolManager2.CreatePool()." +
