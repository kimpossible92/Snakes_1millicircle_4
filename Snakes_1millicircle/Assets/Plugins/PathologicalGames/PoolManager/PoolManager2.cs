...
namespace PathologicalGames
{
    ...
    /// <description>
    /// PoolManager2 v2.0
    ///  - PoolManager2.Pools is not a complete implimentation of the IDictionary interface
    ///    Which enabled:
    ///        * Much more acurate and clear error handling
    ///        * Member access protection so you can't run anything you aren't supposed to.
    ///  - Moved all functions for working with Pools from PoolManager2 to PoolManager2.Pools
    ...
    public class SpawnPoolsDict : IDictionary<string, SpawnPool>
    {
        ...
        /// with the PoolManager2.Pools dictionary. The SpawnPool can then be accessed 
        /// directly via the return value of this function or by via the PoolManager2.Pools 
        ...
        /// itself with the PoolManager2.Pools dictionary. The SpawnPool can then be 
        /// accessed directly via the return value of this function or via the
        /// PoolManager2.Pools dictionary.
        /// </summary>
        /// <param name="poolName">
        /// The name for the new SpawnPool. The GameObject will have the word "Pool"
        /// Added at the end.
        ...
        public bool Destroy(string poolName)
        {
            ...
            {
                Debug.LogError(
                    string.Format("PoolManager2: Unable to destroy '{0}'. Not in PoolManager2",
                                  poolName));
                return false;
            }
        ...
        /// Use PoolManager2.CreatePool() to create an entirely new SpawnPool GameObject
        /// </summary>
        /// <param name="spawnPool"></param>
        ...
        {
            string msg = "SpawnPools add themselves to PoolManager2.Pools when created, so " +
                         "there is no need to Add() them explicitly. Create pools using " +
                         "PoolManager2.Pools.Create() or add a SpawnPool component to a " +
                         "GameObject.";
        ...
        /// Use PoolManager2.Destroy() to destroy an entire SpawnPool GameObject.
        /// </summary>
        /// <param name="spawnPool"></param>
        internal bool Remove(SpawnPool spawnPool)
        {
            if (!this.ContainsValue(spawnPool) & Application.isPlaying)
            {
                Debug.LogError(string.Format(
					"PoolManager2: Unable to remove '{0}'. Pool not in PoolManager2",
                     spawnPool.poolName
				));
                return false;
            }
        ...
        public bool Remove(string poolName)
        {
            string msg = "SpawnPools can only be destroyed, not removed and kept alive" +
                         " outside of PoolManager2. There are only 2 legal ways to destroy " +
                         "a SpawnPool: Destroy the GameObject directly, if you have a " +
                         "reference, or use PoolManager2.Destroy(string poolName).";
            throw new System.NotImplementedException(msg);
        }

        /// <summary>
        /// Get the number of SpawnPools in PoolManager2
        /// </summary>
        ...
        public bool Contains(KeyValuePair<string, SpawnPool> item)
        {
			throw new System.NotImplementedException(
				"Use PoolManager2.Pools.ContainsKey(string poolName) or " +
				"PoolManager2.Pools.ContainsValue(SpawnPool pool) instead."
			);
        }
        ...
        public SpawnPool this[string key]
        {
            ...
            set
            {
                string msg = "Cannot set PoolManager2.Pools[key] directly. " +
                    "SpawnPools add themselves to PoolManager2.Pools when created, so " +
        ...
        public void Clear()
        {
            string msg = "Use PoolManager2.Pools.DestroyAll() instead.";
            throw new System.NotImplementedException(msg);
        ...
        {
            string msg = "PoolManager2.Pools cannot be copied";
            throw new System.NotImplementedException(msg);
        }
